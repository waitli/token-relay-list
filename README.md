...
Token 中转平台导航页，按 `relay`、`source`、`github` 三类展示平台，并定期检查可访问性。GitHub Action 只负责检查平台并更新仓库数据。
...
7. 如果文件有变化，就提交并推送回仓库。
8. Cloudflare Pages 按仓库更新自动部署 `src/` 目录。
...
4. 提交并 push 到 GitHub。
5. 推送到 GitHub 后，让 Cloudflare Pages 自动完成部署。
